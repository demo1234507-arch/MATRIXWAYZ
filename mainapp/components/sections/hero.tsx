  ...
  return (
    <header className="relative flex h-screen min-h-[900px] w-full flex-col items-center justify-center overflow-hidden bg-[#080808] px-4 pb-20 pt-24 text-white">
      ...
      <div className="relative z-30 w-full">
        <p className="mb-6 text-center text-base text-muted-foreground">Trusted by industry leaders</p>
        <div className="w-full overflow-hidden [mask-image:_linear-gradient(to_right,transparent_0,_black_128px,_black_calc(100%-128px),transparent_100%)]">
          <div className="inline-flex w-max flex-nowrap">
            <ul className="flex animate-[infinite-scroll_50s_linear_infinite] items-center justify-center md:justify-start [&_li]:mx-8">
              {[...COMPANY_NAMES, ...COMPANY_NAMES].map((company, index) => (
                <li key={index}>
                  <span className="whitespace-nowrap text-2xl font-semibold tracking-tight text-white/60 transition-colors hover:text-white/80">
                    {company}
                  </span>
                </li>
              ))}
